...
| 18 | 交互聊天的审批请求与成功完成由服务端 `collectStreamResponse` 单点创建 durable event，覆盖三 Runtime。审批按 request id 去重；完成必须同时满足 successful result、assistant terminal 已持久化、仍持有当前 session lock。owner check 到 native event/delivery insert 之间不得有 await；失败、停止、autoTrigger 与 stale owner 不得发完成通知。锁屏正文不得含 tool input、路径或模型正文 | collector + `interactive-chat-notifications.ts` + notification manager static DB insert |
...
- [ ] 交互完成提醒同时检查 successful result、terminal message、lock owner；审批重复 frame 只创建一次且正文不含 tool input。
