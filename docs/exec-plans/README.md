...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/v0.67.1-production-feedback-signing-auto-update-2026-08-23.md](active/v0.67.1-production-feedback-signing-auto-update-2026-08-23.md) | **v0.67.1 生产反馈闭环 × macOS 签名与自动更新（Windows/Linux 手动分发）**：GLM legacy identity、Sentry/SQLite/ABI、macOS 身份门禁、原子 update assets 与 Main-owned updater | 🟡 `v0.67.6` mixed distribution 已 Shipped；`v0.67.7` GLM credential 热修为 release candidate。同一 stable Release 保持 Mac 自动更新资产与 Windows/Linux 手动包，仅发布 `latest-mac.yml`；完整三模型请求与跨版本 smoke 不冒充通过 |
| [active/macos-safe-storage-signing-remediation-2026-08-12.md](active/macos-safe-storage-signing-remediation-2026-08-12.md) | **macOS Safe Storage 反复授权修复**：正式/preview 包改为 Developer ID + 精确 Team ID fail-closed；v0.67.7 不可读 Provider secret 热修阻断 ambient OAuth 串线并提供删除旧服务商、同 Key 重加的恢复入口 | 🟡 Release candidate：Code complete + Tests pass + Build pass；用户已用同一真实 GLM Key 验证删除重加恢复，完整三模型 smoke 作为已接受发版风险保留 |
