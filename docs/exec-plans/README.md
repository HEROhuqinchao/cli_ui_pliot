...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/v0.67.1-production-feedback-signing-auto-update-2026-08-23.md](active/v0.67.1-production-feedback-signing-auto-update-2026-08-23.md) | **v0.67.1 生产反馈闭环 × macOS 签名与自动更新（Windows/Linux 手动分发）**：GLM legacy identity、Sentry/SQLite/ABI、macOS 身份门禁、原子 update assets 与 Main-owned updater | 🚧 `v0.67.6` mixed distribution 修正中；`v0.67.5` Mac stable 已 Shipped。stable 恢复 Windows/Linux 手动包但仅 Mac 发布 updater feed；Windows 当前无 signer，诚实披露 unsigned/SmartScreen。用户将实测 `v0.67.5 → v0.67.6` Mac 自动更新，不冒充 Smoke passed/Release ready |
