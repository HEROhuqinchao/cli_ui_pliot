...
> 创建时间：2026-08-26
> 最后更新：2026-08-26
> 当前状态：🚧 本地实现与测试完成，尚未 Release ready。Windows 采用用户确认过的“无 Authenticode、GitHub 为单一信任根”模型；Linux 保持手动安装并明确延期。GitHub 仓库保护和 Windows RC-A → RC-B 真实升级仍待完成。
...
| Phase 3 | 发布者能看到仓库信任门禁和完整 Windows updater 资产审计 | ⚠️ 代码完成，仓库设置阻断 | 门禁未满足时不公开 `latest.yml`，不把 attestation 冒充客户端强制校验 |
| Phase 4 | Windows RC-A → RC-B 的真实升级结果可验收 | 🔄 本地验证完成，真实 RC 待执行 | Linux updater、包管理器提权安装与 Linux manifest 不进入本轮 |
...
- [x] 仓库设置尚未由用户授权修改前，不执行外部写操作；本地实现完成后把它们列为 Release ready 前置条件。

### Phase 4 — 验证与发布前人验

- [x] Claude 审查修复后重新完成 targeted tests、`npm run test`、production build、Electron bundle 与 workflow YAML 解析。
- [ ] 首个 Windows updater 版本升级版本号时同步当版 `RELEASE_NOTES.md`；不回写已发布的 `v0.67.7` 历史说明。
