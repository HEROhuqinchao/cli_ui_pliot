...
- 2026-08-29：另一台 macOS 设备升级 `v0.67.11` 后，截图证明 selected binary 是 ChatGPT.app 内置 Codex、Runtime candidate 已为 `desktop_bundle`，但 maintenance 显示 Standalone 并执行 self-update。根因是 channel classifier 把所有无扩展名 Codex 都判为 proven standalone；现将 macOS/Windows desktop bundle 识别提前，归 `managed_auto` 且禁用 one-click，加入现场路径与独立 standalone 反例。targeted 11/11、完整 `npm run test` exit 0；dev Renderer 未生成，UI smoke 不冒充通过；本次只提交、不发版。
