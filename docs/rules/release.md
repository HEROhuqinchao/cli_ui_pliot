...
macOS stable 与 preview 都必须把仓库 `MAC_CERT_P12_BASE64` / `MAC_CERT_PASSWORD` secrets 映射为 electron-builder 的 `CSC_LINK` / `CSC_KEY_PASSWORD`，并用 `APPLE_TEAM_ID` → `CODEPILOT_APPLE_TEAM_ID` 校验精确 `TeamIdentifier`。缺 secrets、ad-hoc、Team ID 不匹配、afterSign 或最终产物 `codesign --verify --deep --strict` 失败都必须阻断；禁止上传 ad-hoc 包。无证书的本地目录包只有显式 `CODEPILOT_ALLOW_ADHOC_SIGNING=1` 才允许生成，且只能作隔离开发 smoke，不能标记 `Release ready`。
