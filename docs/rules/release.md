...
3. `push tag` 只表示 CI 已触发，不表示 Shipped。发布者必须跟踪 workflow 终态，并在公开 Release 上复核：非 draft、非 prerelease、Latest=true、版本正确、Linux 手动包齐全、没有 `latest-linux*.yml`、两份 metadata 的 version/URL/sha512/size/blockmap 与公开资产一致。
